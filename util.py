...
def get_rect(im, title='get_rect'):
	...
	while br is None:
		...
		if tl is not None:
			cv2.rectangle(im_draw, tl, current_pos, (0, 0, 255), 4)
...
def track(im_prev, im_gray, keypoints, THR_FB=20):
	...
	if num_keypoints > 0:
		...
		#print keypoints_tracked.shape[0]
		# for k in range(keypoints_tracked.shape[0]):
			# cv2.circle( im_draw,(int(keypoints_tracked[k][0]),int(keypoints_tracked[k][1])),2,(0,255,0),-1)
