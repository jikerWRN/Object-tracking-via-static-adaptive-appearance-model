...
def create_capture(source = 0, fallback = presets['chess']):
    ...
    '''
    source = str(source).strip()    #strip是移除操作
    chunks = source.split(':')
    #handle drive letter ('c:', ...)
    if len(chunks) > 1 and len(chunks[0]) == 1 and chunks[0].isalpha():
        chunks[1] = chunks[0] + ':' + chunks[1]
        del chunks[0]   #删除chunks[0]元素
...
if __name__ == '__main__':
	...
	#短格式——“:”后表示有参数，长格式——“=”后表示有参数，输出：(1)args是一个包含元组的列表，sources是args中没有的参数,(2)短格式输出前面是“-”，长格式是“--”，如：args=[('--shotdir', 'xxx')]
    args = dict(args);   
	#带有参数的转化为字典的键和值，无参数转化为键和''
    shotdir = args.get('--shotdir', '.')
    if len(sources) == 0:
        sources = [ 0 ]
    caps = map(create_capture, sources)  
	#对sources做了create_capture操作，返回列表；即将sources参数代入create_capture函数中
    #caps = "D:\\pycharm\\video\\data\\00060.mp4"
    #print caps
    shot_idx = 0
    while True:
        imgs = []
